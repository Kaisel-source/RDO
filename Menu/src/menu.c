...
void option(int mouseX,int mouseY,SDL_Event event,SDL_Renderer *renderer,SDL_Texture *backgroundTexture) {
            ...
            if (event.type == SDL_QUIT) {
                ...
                if (!settingsPage) {
                    ...
                    if (mouseX >= playButton.rect.x && mouseX <= playButton.rect.x + BUTTON_WIDTH &&
                        ...
                        if (settingsPage == 1){
                            option(mouseX,mouseY,event,renderer,backgroundTexture);
                        } 
